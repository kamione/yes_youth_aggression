...
for (iter in 1:100) {
    ...
    en_best_hyperparamter <- select_best(en_fit_tr, "rmse")
    
    metrics_df[iter, "lambda"] <- pull(en_best_hyperparamter, penalty)
    metrics_df[iter, "mixture"] <- pull(en_best_hyperparamter, mixture)
    
    # model testing
    en_fit_te <- en_workflow %>%
        finalize_workflow(en_best_hyperparamter) %>% 
        last_fit(splits)
...
write_rds(metrics_df, here("outputs", "cache", "model-psy_algo-en_desc-metrics.rds"))
#metrics_df <- read_rds(here("outputs", "cache", "model-psy_algo-en_desc-metrics.rds"))
write_rds(vi_df, here("outputs", "cache", "model-psy_algo-en_desc-model_importance.rds"))
#vi_df <- read_rds(here("outputs", "cache", "model-psy_algo-en_desc-model_importance.rds"))
write_rds(param_df, here("outputs", "cache", "model-psy_algo-en_desc-model_parameters.rds"))
#param_df <- read_rds(here("outputs", "cache", "model-psy_algo-en_desc-model_parameters.rds"))
