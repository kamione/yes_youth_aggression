...
params_psy_df <- 
    here("outputs", "cache", "model-psy_algo-en_desc-model_parameters.rds") %>% 
    read_rds()
params_aggr_df <- 
    here("outputs", "cache", "model-aggr_algo-en_desc-model_parameters.rds") %>% 
    read_rds()
vi_psy_df <- 
    here("outputs", "cache", "model-psy_algo-en_desc-model_importance.rds") %>% 
    read_rds()
vi_aggr_df <- 
    here("outputs", "cache", "model-aggr_algo-en_desc-model_importance.rds") %>% 
    read_rds()
...
write_rds(psy_sorted_feature_names, 
          here("outputs", "cache", "psy_sorted_feature_names_en.rds"))
...
ggsave(filename = here("outputs", "figs", "model-psy_outcome-psy_algo-en_desc-scatter.pdf"), 
       scatter_psy_figure,
       width = 6,
       height = 5)
...
aggr_sorted_feature_names <- aggr_sorted_df %>% 
    filter(exclude == 0) %>% 
    pull(variable)
write_rds(aggr_sorted_feature_names, here("outputs", "cache", "aggr_sorted_feature_names_en.rds"))
...
ggsave(filename = here("outputs", "figs", "model-aggr_outcome-aggr_algo-en_desc-scatter.pdf"), 
       scatter_aggr_figure,
       width = 6,
       height = 5)
...
write_rds(overlapping_features, here("data", "processed", "overlapping_features_en.rds"))
...
# Lasso and Elastic Net Overlapping Features -----------------------------------
overlapping_features_en <- read_rds(here("data", "processed", "overlapping_features_en.rds"))
overlapping_features_lasso <- read_rds(here("data", "processed", "overlapping_features.rds"))
overlapping_features_methods <- intersect(
    overlapping_features_lasso, overlapping_features_en
)
length(overlapping_features_methods)
